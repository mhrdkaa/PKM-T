...
def process_frame_with_yolo(frame, model, frame_width=1280, frame_height=720):
    ...
    try:
        ...
        cv2.putText(frame, "YOLO Error - Original Frame", 
                   (20, frame_height - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        return frame, 0
...
def capture_regular_frame(cam, save_dir="regular_captures", prefix="regular"):
    """Capture frame biasa (tanpa YOLO processing)"""
    os.makedirs(save_dir, exist_ok=True)
    ...
    print(f"Capturing regular frame - Brightness: {frame.mean():.1f}")
    
    # Tambahkan timestamp
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    cv2.putText(frame, f"Regular Camera | {ts}", (18, 40), 
    ...
    if not ok:
        raise RuntimeError("Gagal menyimpan gambar regular.")
    
    print(f"Regular frame saved: {fpath}")
    return fpath
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Load YOLO model di AWAL (opsional, bisa dihapus jika tidak digunakan)
        ...
        # Inisialisasi kamera regular (menggantikan YOLO camera)
        if len(available_cams) > 1:
            regular_camera = init_camera(available_cams[1], camera_name="Regular Camera")
            print("Kamera regular siap")
        else:
            print("Kamera regular tidak tersedia, menggunakan kamera utama untuk semua capture")
            regular_camera = main_camera
            ...
            if found:
                ...
                try:
                    ...
                    if photo_success:
                        ...
                        # 2. Jika COD, proses regular camera + Button
                        if is_cod:
                            print("Paket COD - Menunggu sinyal button dari Arduino...")
                            
                            if tunggu_perintah_dari_arduino():
                                print("Mengambil gambar dari kamera regular...")
                                
                                # 3. Capture gambar regular (menggantikan YOLO)
                                if regular_camera is not None:
                                    regular_path = capture_regular_frame(regular_camera)
                                    print(f"Gambar regular tersimpan: {regular_path}")
                                    
                                    # 4. Kirim gambar regular
                                    regular_caption = f"REGULAR CAMERA\nTime: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                                    regular_success = send_telegram_photos(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, [regular_path], regular_caption)
                                    
                                    if regular_success:
                                        print("Gambar regular berhasil dikirim")
                                        
                                        # 5. Kirim button setelah gambar regular
                                        ...
                                        print("Gagal kirim gambar regular")
                                else:
                                    print("Kamera regular tidak tersedia, langsung kirim button")
                                    # Langsung kirim button jika regular camera tidak tersedia
                                    button_success = send_telegram_buttons(
                                        TELEGRAM_BOT_TOKEN, 
                                        TELEGRAM_CHAT_ID, 
                                        resi_val or data, 
        ...
        if regular_camera and regular_camera.isOpened() and regular_camera != main_camera:
            regular_camera.release()
            print("Kamera regular di-release")
