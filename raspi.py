...
def find_serial_port():
    """Auto-detect serial port yang tersedia"""
    # Cari semua USB serial devices
    usb_ports = glob.glob('/dev/ttyUSB*')
    acm_ports = glob.glob('/dev/ttyACM*')
    
    all_ports = usb_ports + acm_ports
    all_ports.sort()  # Urutkan
    
    print(f"Ports ditemukan: {all_ports}")
    
    # Coba setiap port untuk menemukan yang working
    for port in all_ports:
        try:
            print(f"Mencoba port: {port}")
            # Test dengan baudrate yang sama
            ser_test = serial.Serial(port, 115200, timeout=1)
            ser_test.close()
            print(f"Port {port} berhasil dibuka dan tersedia")
            return port
        except (serial.SerialException, OSError) as e:
            print(f"Port {port} gagal: {e}")
            continue
    
    print("Tidak ada port serial yang available")
    return None

# Auto-detect port di startup
IOT_PORT = find_serial_port()
if IOT_PORT is None:
    # Fallback
    IOT_PORT = "/dev/ttyUSB0"
    print(f"Menggunakan fallback port: {IOT_PORT}")
else:
    print(f"Menggunakan auto-detected port: {IOT_PORT}")
