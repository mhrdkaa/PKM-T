...
def capture_yolo_frame(yolo_cam, yolo_model, save_dir="yolo_captures", prefix="yolo"):
    ...
    fname = f"{prefix}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
    fpath = os.path.join(save_dir, fname)
    ok = cv2.imwrite(fpath, processed_frame, [int(cv2.IMWRITE_JPEG_QUALITY), 90])
...
if __name__ == "__main__":
    ...
    try:
        ...
        yolo_camera_index = 1
        # Inisialisasi kamera YOLO (jika ada kamera kedua)
        if len(available_cams) > 0:
            yolo_camera = init_camera(available_cams[1], camera_name="YOLO Camera")
            print("Kamera YOLO siap")
        else:
            print("Kamera YOLO tidak tersedia")
                    ...
                    if photo_success:
                        ...
                        if is_cod:
                            ...
                            if tunggu_perintah_dari_arduino():
                                ...
                                if yolo_camera is not None and yolo_model is not None:
                                    ...
                                    print("Kamera YOLO tidak tersedia, langsung kirim button")
        ...
        if yolo_camera and yolo_camera.isOpened():
            yolo_camera.release()
